...
export async function saveMatchHistory(match: MatchHistoryDetail): Promise<void> {
  try {
    // 儲存詳情
    await AsyncStorage.setItem(STORAGE_KEYS.HISTORY_DETAIL(match.id), JSON.stringify(match));
...
export async function deleteMatchHistory(id: string): Promise<void> {
  try {
    ...
    await AsyncStorage.setItem(STORAGE_KEYS.HISTORY_LIST, JSON.stringify(filtered));
  } catch (error) {
...
export async function clearAllHistory(): Promise<void> {
  try {
    const list = await getHistoryList();
    await Promise.all(
      list.map(h => AsyncStorage.removeItem(STORAGE_KEYS.HISTORY_DETAIL(h.id)))
    );
    await AsyncStorage.removeItem(STORAGE_KEYS.HISTORY_LIST);
  } catch (error) {
